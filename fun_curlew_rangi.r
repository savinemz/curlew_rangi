...
f_shp_rangi <- function(shp_processing) {
    ...
    ## immerge_rocks = atoll - ( ocean  + lagon + lagon_bleu + land)
    ## beach = land - (platier + foret_dense + foret_clair + vasiere + bush + rocks )
    ## reef = platier -  (foret_dense + foret_clair + vasiere + bush + rocks)
    ## foret_clair = foret_clair - (foret_dense + vasiere + bush)
    ## bush = bush - (platier + foret_dense + foret_clair + rocks)

  

    if(shp_processing) {
        shp_name <-   c("atoll","ocean","blue_lagoon","lagoon","land","reef","bush","forest_dense","forest_clairseme","mudflat","rocks")
    ...
    vec_habitat <- c("ocean","blue_lagoon","lagoon","immerge_rocks","beach","reef","bush","forest_dense","forest_clairseme","mudflat","rocks")
